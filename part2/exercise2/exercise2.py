...
def initialize_random_weights(in_conn, out_conn):
    epsilon_init = 0.12
    return np.random.rand(out_conn, 1 + in_conn) * 2 * epsilon_init - epsilon_init
...
res = minimize(nn_cost_function, init_params, args=args, method="CG", callback=callbackF, jac=True,
